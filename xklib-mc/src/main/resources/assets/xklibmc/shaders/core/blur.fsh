...
float weight2 = gaussianPdf(2.0,Radius);
float weightSum = weight0 + weight1 * 8.0 + weight2 * 16.0;
...
void main(){
    ...
    diffuseSum += texture(input0, texCoord + vec2( 2.0,-1.0) * invSize).rgb * weight2;
    diffuseSum += texture(input0, texCoord + vec2( 2.0, 0.0) * invSize).rgb * weight2;
    diffuseSum += texture(input0, texCoord + vec2( 2.0, 1.0) * invSize).rgb * weight2;
    diffuseSum += texture(input0, texCoord + vec2( 2.0, 2.0) * invSize).rgb * weight2;

    diffuseSum += texture(input0, texCoord + vec2( 1.0, 2.0) * invSize).rgb * weight2;
    diffuseSum += texture(input0, texCoord + vec2( 0.0, 2.0) * invSize).rgb * weight2;
    diffuseSum += texture(input0, texCoord + vec2(-1.0, 2.0) * invSize).rgb * weight2;
    diffuseSum += texture(input0, texCoord + vec2(-2.0, 2.0) * invSize).rgb * weight2;
