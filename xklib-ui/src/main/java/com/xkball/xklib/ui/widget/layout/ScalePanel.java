...
public class ScalePanel extends AbstractContainerWidget<ScalePanel, ScalePanel.NoParma> {
    ...
    protected boolean allowUserScale = true;
    ...
    public ScalePanel setAllowUserScale(boolean allow) {
        this.allowUserScale = allow;
        if (!this.allowUserScale) {
    ...
    public ScalePanel setScale(double value) {
        double target = clampScale(value);
    ...
    public ScalePanel setMinScale(double value) {
        this.minScale = value;
        double clamped = clampScale(this.scale);
    ...
    public ScalePanel setScaleKeepCenter(double value) {
        double target = clampScale(value);
        if (target == this.scale) {
            return this;
        }
        if (this.child == null || this.contentWidth <= 0 || this.contentHeight <= 0) {
            this.scale = target;
            this.markDirty();
            return this;
        }
        double centerX = this.contentX + this.contentWidth / 2.0;
        double centerY = this.contentY + this.contentHeight / 2.0;
        double cx = this.toChildX(centerX);
        double cy = this.toChildY(centerY);
        this.scale = target;
        this.offsetX = centerX - this.contentX - cx * this.scale;
        this.offsetY = centerY - this.contentY - cy * this.scale;
        this.markDirty();
        return this;
    }
    ...
    public boolean mouseClicked(IMouseButtonEvent event, boolean doubleClick) {
        ...
        if (event.button() == 0 && this.allowUserScale) {
    ...
    public boolean mouseDragged(IMouseButtonEvent event, double dx, double dy) {
        ...
        if (this.draggingPanel && event.button() == 0 && this.allowUserScale) {
            if (dx == 0.0 && dy == 0.0) {
                return true;
            }
            this.offsetX += dx;
